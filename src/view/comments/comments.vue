<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="upNum字段" prop="upNum" width="120" />
        <el-table-column align="left" label="评论内容" prop="content" width="120" />
        <el-table-column align="left" label="状态" prop="status" width="120" />
        <el-table-column sortable align="left" label="评论目标类型" prop="targetType" width="120" />
        <el-table-column sortable align="left" label="评论目标id" prop="targetId" width="120" />
        <el-table-column align="left" label="pid字段" prop="pid" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCommentsFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getCommentsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCommentsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCommentsFunc = async(row) => {
    const res = await findComments({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCommentsFunc = async (row) => {
