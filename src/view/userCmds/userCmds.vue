<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="命令" prop="cmd" width="240" />
        <el-table-column align="left" label="工作目录" prop="wrokDir" width="240" />
        <!-- <el-table-column align="left" label="环境变量" prop="envs" width="120" /> -->
        <el-table-column align="left" label="是否保活" prop="keepAlive" width="80">
        ...
        <el-table-column align="left" label="类型" prop="type" width="80">
            ...
            {{ filterDict(scope.row.type,user_cmd_typeOptions) }}
            </template>
        </el-table-column>
        <!-- <el-table-column align="left" label="描述信息" prop="desc" width="120" /> -->
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateUserCmdsFunc(scope.row)">变更</el-button>
            <el-button type="primary" link icon="edit" class="table-button" @click="copyUserCmdsFunc(scope.row)">复制</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button v-if="type!=='copy'" type="primary" @click="enterDialog">确 定</el-button>
          <el-button v-else type="primary" @click="enterDialog">复制</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const getTableData = async() => {
  const table = await getUserCmdsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteUserCmdsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserCmdsFunc = async(row) => {
    const res = await findUserCmds({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserCmdsFunc = async (row) => {
    const res = await deleteUserCmds({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
